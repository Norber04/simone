...
void fsm_keyboard_init(fsm_keyboard_t *p_fsm_keyboard, uint8_t keyboard_id)
{
    // Initialize the FSM
...
/* Public functions -----------------------------------------------------------*/
fsm_keyboard_t *fsm_keyboard_new(uint8_t keyboard_id)
{
...
static bool check_row_timeout (fsm_t *p_this){
    fsm_keyboard_t *fsm_keyboard =(fsm_keyboard_t *)p_this;
    stm32f4_keyboard_hw_t *p_keyboard = _stm32f4_keyboard_get(fsm_keyboard->keyboard_id);
