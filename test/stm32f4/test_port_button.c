...
/* Defines and enums ----------------------------------------------------------*/
/* Defines */
#define TEST_PORT_USER_BUTTON_ID 0   /*!< Button identifier @hideinitializer */
#define LD2_PORT GPIOA
#define LD2_PIN 5
#define LD2_DELAY_MS 100
#define TEST_STM32F4_USER_BUTTON_GPIO GPIOC
#define TEST_STM32F4_USER_BUTTON_PIN 13
#define TEST_STM32F4_USER_BUTTON_EXTI_IRQn EXTI15_10_IRQn
#define TEST_IRQ_PRIORITY 1 /*!< Expected IRQ priority for USER BUTTON EXTI IRQ @hideinitializer */
#define TEST_IRQ_SUBPRIORITY 0 /*!< Expected IRQ subpriority for USER BUTTON EXTI IRQ @hideinitializer */
...
{
    UNITY_TEST_ASSERT_EQUAL_INT(TEST_PORT_USER_BUTTON_ID, PORT_USER_BUTTON_ID, __LINE__, "ERROR: PORT_USER_BUTTON_ID must be is incorrect");
}

void test_button_wiring(void)
{
    UNITY_TEST_ASSERT_EQUAL_INT(TEST_STM32F4_USER_BUTTON_GPIO, STM32F4_USER_BUTTON_GPIO, __LINE__, "ERROR: USER_BUTTON GPIO is incorrect");
    UNITY_TEST_ASSERT_EQUAL_INT(TEST_STM32F4_USER_BUTTON_PIN, STM32F4_USER_BUTTON_PIN, __LINE__, "ERROR: USER_BUTTON pin is incorrect");
}

void _test_regs_config(void)
{
    // Retrieve previous configuration
    uint32_t prev_gpio_mode = STM32F4_USER_BUTTON_GPIO->MODER;
    uint32_t prev_gpio_pupd = STM32F4_USER_BUTTON_GPIO->PUPDR;

    // Call configuration function
    port_button_init(TEST_PORT_USER_BUTTON_ID);

    // Check that the mode is configured correctly
    uint32_t button_mode = ((STM32F4_USER_BUTTON_GPIO->MODER) >> (STM32F4_USER_BUTTON_PIN * 2)) & GPIO_MODER_MODER0_Msk;
    ...
    uint32_t button_pupd = ((STM32F4_USER_BUTTON_GPIO->PUPDR) >> (STM32F4_USER_BUTTON_PIN * 2)) & GPIO_PUPDR_PUPD0_Msk;
    ...
    uint32_t mask = ~(GPIO_MODER_MODER0_Msk << (STM32F4_USER_BUTTON_PIN * 2));
    uint32_t prev_gpio_mode_masked = prev_gpio_mode & mask;

    mask = ~(GPIO_PUPDR_PUPD0_Msk << (STM32F4_USER_BUTTON_PIN * 2));
    uint32_t prev_gpio_pupd_masked = prev_gpio_pupd & mask;

    uint32_t curr_gpio_mode_masked = STM32F4_USER_BUTTON_GPIO->MODER & mask;
    uint32_t curr_gpio_pupd_masked = STM32F4_USER_BUTTON_GPIO->PUPDR & mask;
...
{
    // Retrieve previous configuration
    uint32_t prev_gpio_exticr = SYSCFG->EXTICR[STM32F4_USER_BUTTON_PIN / 4];
    ...
    uint32_t button_exticr = ((SYSCFG->EXTICR[STM32F4_USER_BUTTON_PIN / 4]) >> ((STM32F4_USER_BUTTON_PIN % 4) * 4)) & 0xF;
    ...
    uint32_t button_rtsr = ((EXTI->RTSR) >> STM32F4_USER_BUTTON_PIN) & 0x1;
    ...
    uint32_t button_ftsr = ((EXTI->FTSR) >> STM32F4_USER_BUTTON_PIN) & 0x1;
    ...
    uint32_t button_emr = ((EXTI->EMR) >> STM32F4_USER_BUTTON_PIN) & 0x1;
    ...
    uint32_t button_imr = ((EXTI->IMR) >> STM32F4_USER_BUTTON_PIN) & 0x1;
    ...
    // Check that no other pins other than the needed have been modified:
    uint32_t mask_exticr = ~(0xF << ((STM32F4_USER_BUTTON_PIN % 4) * 4));
    uint32_t prev_gpio_exticr_masked = prev_gpio_exticr & mask_exticr;
    uint32_t curr_gpio_exticr_masked = SYSCFG->EXTICR[STM32F4_USER_BUTTON_PIN / 4] & mask_exticr;
    ...
    uint32_t prev_gpio_rtsr_masked = prev_gpio_rtsr & ~(EXTI_RTSR_TR0_Msk << STM32F4_USER_BUTTON_PIN);
    uint32_t prev_gpio_ftsr_masked = prev_gpio_ftsr & ~(EXTI_FTSR_TR0_Msk << STM32F4_USER_BUTTON_PIN);
    uint32_t curr_gpio_rtsr_masked = EXTI->RTSR & ~(EXTI_RTSR_TR0_Msk << STM32F4_USER_BUTTON_PIN);
    uint32_t curr_gpio_ftsr_masked = EXTI->FTSR & ~(EXTI_FTSR_TR0_Msk << STM32F4_USER_BUTTON_PIN);
    ...
    uint32_t prev_gpio_emr_masked = prev_gpio_emr & ~(EXTI_EMR_MR0_Msk << STM32F4_USER_BUTTON_PIN);
    uint32_t prev_gpio_imr_masked = prev_gpio_imr & ~(EXTI_IMR_MR0_Msk << STM32F4_USER_BUTTON_PIN);
    uint32_t curr_gpio_emr_masked = EXTI->EMR & ~(EXTI_EMR_MR0_Msk << STM32F4_USER_BUTTON_PIN);
    uint32_t curr_gpio_imr_masked = EXTI->IMR & ~(EXTI_IMR_MR0_Msk << STM32F4_USER_BUTTON_PIN);
...
void test_exti(void)
{
    SYSCFG->EXTICR[STM32F4_USER_BUTTON_PIN / 4] = ~0;
    EXTI->RTSR = ~0;
    EXTI->FTSR = ~0;
    EXTI->EMR = ~0;
    EXTI->IMR = ~0;
...
/**
 * @brief Test the NVIC priority of the EXTI IRQ line associated with the USER BUTTON.
 *
 */
void test_exti_enabled_priority(void)
{
    ...
    // Check that the NVIC IRQ is enabled
    UNITY_TEST_ASSERT_EQUAL_UINT8(true, nvic_irq_enabled(TEST_STM32F4_USER_BUTTON_EXTI_IRQn), __LINE__, "ERROR: NVIC EXTI IRQ for USER BUTTON is not enabled");

    uint32_t Priority = NVIC_GetPriority(TEST_STM32F4_USER_BUTTON_EXTI_IRQn);
    ...
    UNITY_TEST_ASSERT_EQUAL_UINT32(TEST_IRQ_PRIORITY, pPreemptPriority, __LINE__, "ERROR: NVIC EXTI IRQ priority for USER BUTTON is not correct");
    UNITY_TEST_ASSERT_EQUAL_UINT32(TEST_IRQ_SUBPRIORITY, pSubPriority, __LINE__, "ERROR: NVIC EXTI IRQ subpriority for USER BUTTON is not correct");
}
...
void test_button_port_generalization(void)
{
    ...
    if (STM32F4_USER_BUTTON_GPIO == GPIOA)
    {
        RCC->AHB1ENR |= RCC_AHB1ENR_GPIOAEN; /* GPIOA_CLK_ENABLE */
    }
    else if (STM32F4_USER_BUTTON_GPIO == GPIOB)
    {
        RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN; /* GPIOB_CLK_ENABLE */
    }
    else if (STM32F4_USER_BUTTON_GPIO == GPIOC)
    {
    ...
    STM32F4_USER_BUTTON_GPIO->MODER |= (STM32F4_GPIO_MODE_OUT << (STM32F4_USER_BUTTON_PIN * 2U));  // Wrong configuration to check that the GPIO is not being modified
    STM32F4_USER_BUTTON_GPIO->PUPDR |= (STM32F4_GPIO_PUPDR_PULLUP << (STM32F4_USER_BUTTON_PIN * 2U)); // Wrong configuration to check that the GPIO is not being modified
    ...
    // Real configuration of the GPIO    
    ...
    uint32_t real_rtsr = EXTI->RTSR;
    uint32_t real_ftsr = EXTI->FTSR;
    uint32_t real_emr = EXTI->EMR;
    ...
    uint32_t curr_gpio_mode = STM32F4_USER_BUTTON_GPIO->MODER;
    uint32_t curr_gpio_pupd = STM32F4_USER_BUTTON_GPIO->PUPDR;
    uint32_t curr_exticr = SYSCFG->EXTICR[STM32F4_USER_BUTTON_PIN / 4];
    ...
    uint32_t curr_priority = NVIC_GetPriority(TEST_STM32F4_USER_BUTTON_EXTI_IRQn);

    UNITY_TEST_ASSERT_EQUAL_UINT32(real_gpio_mode, curr_gpio_mode, __LINE__, "ERROR: The configuration function is not generalizing the GPIO and/or pin but working with the specific GPIO and pin");
    UNITY_TEST_ASSERT_EQUAL_UINT32(real_gpio_pupd, curr_gpio_pupd, __LINE__, "ERROR: The configuration function is not generalizing the GPIO and/or pin but working with the specific GPIO and pin");
    UNITY_TEST_ASSERT_EQUAL_UINT32(real_exticr, curr_exticr, __LINE__, "ERROR: The configuration function is not generalizing the GPIO and/or pin but working with the specific GPIO and pin");
    UNITY_TEST_ASSERT_EQUAL_UINT32(real_rtsr, curr_rtsr, __LINE__, "ERROR: The configuration function is not generalizing the GPIO and/or pin but working with the specific GPIO and pin");
    UNITY_TEST_ASSERT_EQUAL_UINT32(real_ftsr, curr_ftsr, __LINE__, "ERROR: The configuration function is not generalizing the GPIO and/or pin but working with the specific GPIO and pin");
