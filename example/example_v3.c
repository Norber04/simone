...
int main(void)
{
    ...
    while (1)
    {
        ...
        // Set a new color and intensity
        printf("Testing color [R, G, B] = [%d, %d, %d]\n", r, g, b);
        uint8_t intensity_perc = MAX_LEVEL_INTENSITY;
        
        fsm_rgb_light_set_color_intensity(p_fsm_rgb_light, color_test, intensity_perc);
        ...
        while((p_fsm_rgb_light->color.r > 1) && (p_fsm_rgb_light->color.g > 1) && (p_fsm_rgb_light->color.b > 1))
        {
            ...
            fsm_rgb_light_set_color_intensity(p_fsm_rgb_light, p_fsm_rgb_light->color, intensity_perc);
            fsm_rgb_light_fire(p_fsm_rgb_light);
